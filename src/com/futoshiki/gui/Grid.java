...
/*
 * C'est la classe qui a réuni toutes les classes pour faire un 
 * travail puzzle futoshiki.
 *  Dans cette classe, l'impression de remplissage et le contrôle sont tous terminé.*/
...
public class Grid implements Serializable {
	...
	/*
	 * public static void main(String[] args) { FutoshikiGrid x = new
	 * FutoshikiGrid(5); x.fillPuzzle(); System.out.println(x); x.isLegal(); }
	 */
	/**
	 ...
	 * @param row is the indexed row of the constraint that you want.
	 * @param col is the indexed column of the constraint that you want.
	 * @return returns the constraint in the square where the row and col intersect.
	 */
	...
	/*
	 * This method will return teh numbers of a specified square that the square can
	 * not contain.
	 *
	 ...
	 * @param input is the number you want to be put in the square at (row, col), it
	 *              has to be greater than 0 and less than gridSize.
	 */
	...
	/**
	 * Adds a number to the ArrayList in a square about which numbers the square can
	 * not be. It has to meet the conditions which make sure it lies within the size
	 * of the grid and the notNum has to be greater than 1 and less than gridSize.
	 *
	...
	/**
	 * Adds a constraint to a square in the row constraints grid. It has to meet the
	 * conditions which make sure it lies within the size of the grid. Depending on
	 * the constraint it will make either a GreaterThan or LessThan object.
	 *
	 * @param row        is the indexed row of the square that you want add the
	 *                   number it cant be to.
	 * @param col        is the indexed col of the square that you want add the
	 *                   number it cant be to.
	 * @param constraint the constraint you want to set either ">" or "<".
	 *
	 */
	public void setRowConstraint(int row, int col, String constraint) {
		if (row < 0 || row >= gridSize || col < 0 || col >= gridSize) {// equal to 0 so you can make it so a box is
																		// empty
			System.err.println("Out of range of the indexed grid");
	...
	/**
	 * Adds a constraint to a square in the row constraints grid. It has to meet the
	 * conditions which make sure it lies within the size of the grid.
	 *
	 * @param row        is the indexed row of the square that you want add the
	 *                   number it cant be to.
	 * @param col        is the indexed col of the square that you want add the
	...
	/**
	 * Adds the relevant values to each square. It give numbers between 1 and 5 for
	 * the numbers and different constraints for the constraints. it picks a number
	 * of random squares adds it to them
	 */
	...
	/**
	 * Checks that all the number in the numGrid horizontally are unique. If there
	 * is duplicates it will print out what number is duplicated and how many times
	 * it is duplicated.
	 *
	...
	public String checkRowNum() {
		...
		// turned to 0 so it is not checked again, doing this without effecting the
		// orignal grid
		...
		for (int i = 0; i < gridSize; i++) {
			for (int j = 0; j < gridSize; j++) {
				if (tmp[i][j].getNumber() != 0) {
					...
					if (tooMany > 0) {
						problem += "Row " + (i + 1) + " has " + tooMany + " too many " + getSquare(i, j).getNumber()
								+ "'s \n";
					}
				}
			}
	...
	/**
	 * Checks that all the number in the numGrid vertically are unique. If there is
	 * duplicates it will print out what number is duplicated and how many times it
	 * is duplicated.
	 *
	...
	public String checkColNum() {
		String problem = "";
		Square[][] tmp = new Square[gridSize][gridSize]; // use this so that when a number is checked it is
		// turned to 0 so it is checked again, do this without effecting the orignal
		// grid
		...
		for (int i = 0; i < gridSize; i++) {
			for (int j = 0; j < gridSize; j++) {
				if (tmp[j][i].getNumber() != 0) {
					...
					if (tooMany > 0) {
						problem += "Column " + (i + 1) + " has " + tooMany + " too many " + getSquare(j, i).getNumber()
								+ "'s \n";
					}
				}
			}
	...
	/**
	 * Checks that all the row constraints are met and if any aren't then it will
	 * keep record of the problems.
	 *
	...
	public String checkRowConstraints() {
		...
		for (int i = 0; i < gridSize; i++) {
			for (int j = 0; j < gridSize - 1; j++) {
				if ((getRowConstraint(i, j).constraint() == ("<") && getSquare(i, j).getNumber() == gridSize) || // check
																													// if
																													// a
																													// problem
																													// where
				...
				if (getRowConstraint(i, j) != null && getSquare(i, j).getNumber() != 0
						&& getSquare(i, j + 1).getNumber() != 0) {
					int tooMany = 0;
					if (!getRowConstraint(i, j).isCorrect()) {
						tooMany++;
					}
					if (tooMany > 0) {
						problem += "In row " + (i + 1) + ", " + getSquare(i, j).getNumber() + " is not "
								+ getRowConstraint(i, j).stringRep() + " " + getSquare(i, j + 1).getNumber() + "\n";
					}
				}
			}
		}
	...
	/**
	 * Checks that all the column constraints are met and if any aren't then it will
	 * keep record of all the problems.
	 *
	 * @return all the problems where the constraints aren't met in the rows.
	 */
	public String checkColConstraints() {
		...
		for (int j = 0; j < gridSize; j++) {
			for (int i = 0; i < gridSize - 1; i++) {
				if ((getColConstraint(i, j).constraint() == ("^") && getSquare(i, j).getNumber() == gridSize) || // check
																													// if
																													// asking
																													// for
																													// something
				...
				if (getColConstraint(i, j).constraint() == ("V") && getSquare(i, j).getNumber() == 1 || // check if were
																										// asking for
																										// something
																										// less than 1
				...
				if (getSquare(i, j).getNumber() != 0 && getSquare(i + 1, j).getNumber() != 0
						&& getColConstraint(i, j) != null) {
					int tooMany = 0;
					if (!(getColConstraint(i, j).isCorrect())) {
						tooMany++;
					}
					if (tooMany > 0) {
						problem += "In column " + (j + 1) + ", " + getSquare(i, j).getNumber() + " is not "
								+ getColConstraint(i, j).stringRep() + " " + getSquare(i + 1, j).getNumber() + "\n"; // sort
																														// out
																														// signs
																														// when
																														// printing
	...
	/**
	 * If the problems field is empty then it means that there are no problems and
	 * it is a legal puzzle, otherwise it means that there are problems and it will
	 * return the problems.
	 *
	 * @return a string of all the problems if there is any otherwise "Legal puzzle"
	 *         is printed.
	 */
	...
	/**
	 * This will print out the grid in a suitable representation in an actual gird.
	 * It will print all the numbers and Constraints in the correct place. The 0
	 * will be printed as a space
	 *
